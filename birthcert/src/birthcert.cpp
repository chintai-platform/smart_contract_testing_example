...
void birthcert::add_birth_certificate(uint64_t const certificate_id, eosio::name const & account, std::string const & full_name, eosio::time_point const & date_of_birth)
{
...
void birthcert::validate(eosio::name const & account, std::string const & full_name, eosio::time_point const & date_of_birth)
{
...
void birthcert::insert(uint64_t const certificate_id, eosio::name const & account, std::string const & full_name, eosio::time_point const & date_of_birth)
{
...
void birthcert::set_retirement_date(eosio::name const & account, eosio::time_point const & date_of_birth)
{
  eosio::time_point const retirement_date = calculate_retirement_date(date_of_birth);
  eosio::name const retirement_contract = retirement_contract_table(get_self(), get_self().value).require_find(0, "The retirement contract has not been set")->get_account();
  retirement::set_retirement_date_action(retirement_contract, { get_self(), "active"_n }).send(account, retirement_date);
}

eosio::time_point birthcert::calculate_retirement_date(eosio::time_point const & date_of_birth)
{
  return eosio::time_point(eosio::microseconds( (static_cast<int64_t>(date_of_birth.sec_since_epoch()) + 65*365*24*60*60) * 1000000) );
}

void birthcert::set_retirement_contract(eosio::name const & account)
{
  require_auth(get_self());
  retirement_contract_table table(get_self(), get_self().value);
